...
function EmptyState({ onPick, suggestions }: { onPick: (text: string) => void; suggestions: string[] }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
      <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-600 text-sm font-semibold text-white shadow-sm">AI</div>
      <h2 className="mt-3 text-sm font-semibold text-neutral-900 dark:text-neutral-100">How can I help you shop today?</h2>
...
export function ChatWidget({
  ...
  return (
    <>
      ...
      >
        {isOpen ? <CloseIcon className="h-5 w-5" /> : <ChatBubbleIcon className="h-6 w-6" />}
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-30 flex items-end justify-end sm:inset-auto sm:bottom-24 sm:right-5">
          <div className="flex h-full w-full flex-col bg-white shadow-2xl sm:h-[600px] sm:w-96 sm:rounded-2xl sm:border sm:border-neutral-200 dark:bg-neutral-950 sm:dark:border-neutral-800">
            <header className="flex shrink-0 items-center gap-3 border-b border-neutral-200 px-4 py-3 dark:border-neutral-800">
              ...
              <button
                ...
                aria-label="Close"
                className="flex h-7 w-7 items-center justify-center rounded-full text-neutral-400 hover:bg-neutral-100 hover:text-neutral-600 dark:hover:bg-neutral-900 sm:hidden"
              >
            ...
            {error && (
              <div className="flex items-center justify-between gap-3 border-t border-red-100 bg-red-50 px-4 py-2 text-xs text-red-700 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-400">
